...
sig
  ...
  (*
   * From a list of basic blocks satisfying properties 1-6, along with an
   * exit label, produce a list of stms such that:
   *
   * 1. No `SEQ`s or `ESEQ`s
   * 2. The parent of every `CALL` is an `EXP` or a `MOVE (TEMP t, ...)`
   ...
   * as many `JUMP (T.NAME lab)` statements as possible are eliminated by
   * falling through into `T.LABEL lab`.
   *)
