...
import Character.{ digit, isDigit, isWhitespace }
...
object Scanner {
  ...
  private def consume[C, S](reader: Reader[C, S], stream: S): (List[C], S) = {
    ...
    def recur(stream: S, result: Vector[C]): (List[C], S) = {
      val next = reader(stream)

      if (next.isDefined) {
        val (c, newStream) = next.get
        recur(newStream, result :+ c)
      } else {
  ...
  private def isLetter(c: Char): Boolean = {
    (c >= 'a'.toInt && c <= 'z'.toInt) || (c >= 'A'.toInt && c <= 'Z'.toInt)
  }
